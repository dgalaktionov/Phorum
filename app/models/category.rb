class Category < ActiveRecord::Base
  ...
  def self.stats_for_index
    ActiveRecord::Base.connection.execute("SELECT c.id as id, c.name as name, count(distinct t.id) as topics, count(distinct p.id) as posts, c.updated_at, (SELECT id FROM topics WHERE category_id = c.id AND updated_at = max(t.updated_at) LIMIT 1) as last_topic, (SELECT name FROM topics WHERE updated_at = max(t.updated_at) LIMIT 1) as last_topic_name,  (SELECT id FROM posts WHERE topic_id = (SELECT id FROM topics WHERE updated_at = max(t.updated_at) LIMIT 1) ORDER BY updated_at DESC LIMIT 1) as last_post FROM (categories as c LEFT JOIN topics as t ON c.id = t.category_id) LEFT JOIN posts as p ON t.id = p.topic_id GROUP BY c.id")
  end
